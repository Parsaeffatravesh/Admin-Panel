...
func (s *AuthService) Logout(ctx context.Context, userID uuid.UUID, ipAddress, userAgent string) error {
        ...
        if err := s.sessionRepo.RevokeByUserID(ctx, userID, time.Now()); err != nil {
                return err
        }
...
func (s *AuthService) RefreshTokens(ctx context.Context, refreshToken string) (*AuthTokens, error) {
        ...
        if session.RotatedAt != nil || session.ReplacedByToken != nil {
                if err := s.sessionRepo.RevokeByUserID(ctx, session.UserID, time.Now()); err != nil {
                        return nil, err
                }
        ...
        now := time.Now()
        if err := s.sessionRepo.MarkRotated(ctx, session.ID, now, tokens.RefreshToken); err != nil {
                return nil, err
        }
